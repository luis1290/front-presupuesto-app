...
const GetSpentDataRange = ({ themeGlobal }) => {
  ...
  const handleSearch = () => {
    ...
    axios.post(`http://localhost:8000/getspentdatarange/${id}`, formValues, getConfig())
      .then((response) => {
        ...
        if (Array.isArray(response.data.rangeDateSpent)) {
          const filteredItems = response.data.rangeDateSpent.filter((item) =>
            item.name.toLowerCase().includes(searchTerm.toLowerCase())
          );
          setFilteredItems(filteredItems);
        } else {
          // Manejar caso donde response.data no es un arreglo
          Swal.fire('La respuesta de la API no es un arreglo válido:', response.data)
        ...
        <Container sx={{ py: 8 }} maxWidth="md">
          ...
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {Array.isArray(currentItems) && currentItems.map((spen) => (
                  <TableRow key={spen?.id}>
                    <TableCell>{spen?.name}</TableCell>
                    <TableCell>{spen?.description}</TableCell>
                    <TableCell>{formatCurrency(spen?.amount)}</TableCell>
                    <TableCell>{converDate(spen?.createdAt)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
export default GetSpentDataRange;